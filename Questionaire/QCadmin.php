<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="style.css">
  <title>Document</title>
</head>
<body>
  <div class="container">

    <div class="box1-container">

      <div class="links-box1">
        <a href="homePage.php">create a questionaire</a>
        <a href="MultipleChoice Questionaire/MCQ.php"> answer the multiple choice questionaire</a>
        <a href="yes_NoQuestionaire/yes_no.php"> answer the yes/no questionaire</a>
        <a href="short Questionaire/short.php"> answer the short questionaire</a>
        <a href="range questionaire/range.php"> answer the range questionaire</a>

        <a href="reports.php" target="_blank"> see report for all questionaires</a>

        



      </div>

    </div>

  </div>


</body>
</html>